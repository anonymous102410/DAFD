...
class LMMDLoss(MMDLoss, LambdaSheduler):
    def __init__(self, num_class, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None, 
                    gamma=1.0, max_iter=1000, dynamic = True,d_lamb = 1,**kwargs):
        '''
        Local MMD
        '''
    ...
    def forward(self, source, target, source_label, target_logits):
        if self.kernel_type == 'linear':
            ...
            loss = loss * lamb
            return loss
    def info_entro(self,input):
        return 1/(-(input*torch.log(input)).sum(1))
    def cal_weight(self, source_label, target_logits):
        ...
        source_label_onehot = source_label_onehot / source_label_sum # label ratio #(B,class_num)

        # Pseudo label
        target_label = target_logits.cpu().data.max(1)[1].numpy()
        if self.dynamic !=0 :
            target_info_entro = self.info_entro(target_logits).unsqueeze(1).repeat(1,self.num_class)
            target_logits = target_logits * target_info_entro * self.d_lamb
