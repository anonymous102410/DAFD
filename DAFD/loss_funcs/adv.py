...
class AdversarialLoss(nn.Module):
    ...
    def get_adversarial_result(self, x, source=True, lamb=1.0):
        ...
        return loss_adv
    def get_acc(self,label,predic):
        predic[predic[:,0]>=0.5] = 1
        predic[predic[:,0]<0.5] = 0
        predic = predic.long()
        correct_num = torch.sum(predic==label).detach().cpu().numpy()
        acc = correct_num/predic.shape[0] * 100.0
        return acc
...
class Discriminator(nn.Module):
    def __init__(self, input_dim=256, hidden_dim=256*2):
